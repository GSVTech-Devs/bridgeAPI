...
Validates what mocks can't: SQL aggregation actually executes on Postgres,
``error_rate`` is a percentage, ``since``/``until`` narrow at the DB level,
and the account dashboard is scoped to the caller while admin sees the global
total.
"""
...
async def _seed_key(db: AsyncSession, account_id: uuid.UUID, prefix: str) -> APIKey:
    key = APIKey(
...
async def test_account_dashboard_aggregates_real_metrics_correctly(
    ...
    response = await client.get("/metrics/dashboard", headers=account_headers(acme.id))
...
async def test_account_dashboard_scopes_to_caller_only(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    ...
    acme_body = (
        await client.get("/metrics/dashboard", headers=account_headers(acme.id))
    ).json()
    other_body = (
        await client.get("/metrics/dashboard", headers=account_headers(other.id))
    ).json()
...
async def test_admin_global_metrics_sums_all_accounts(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    acme, _ = await seed_account(db_session, email="acme@example.com", name="Acme")
    other, _ = await seed_account(db_session, email="other@example.com", name="Other")
...
async def test_since_until_filters_narrow_window_at_db_level(
    ...
    narrow = await client.get(
        "/metrics/dashboard", params={"since": cutoff}, headers=account_headers(acme.id)
    )
    wide = await client.get("/metrics/dashboard", headers=account_headers(acme.id))

    assert narrow.json()["total_requests"] == 2
    assert wide.json()["total_requests"] == 3

    upper = (now - timedelta(days=5)).isoformat()
    capped = await client.get(
        "/metrics/dashboard", params={"until": upper}, headers=account_headers(acme.id)
    )
    assert capped.json()["total_requests"] == 1
